...
def assign_Task(project_id,task_id,user_id):
    if project_id not in projects:
...
def Genarate_project_timeline(project_id):
    if project_id in projects:
        ...
        if project_tasks:
            ...
            for task_id, task_info in project_tasks.items():
                ...
                print(f"  Task ID: {task_id}, Name: {task_info['name']}, Assigned User: {task_info['assigned_user']}, Status: {task_info['status']}")
...
def update_project_status():
    ...
    if project_id in projects:
        ...
        if new_status in ["Not Started", "In Progress", "Completed"]:
            ...
            print(f"Project {project_id} status updated to {new_status} at {(datetime.now() + timedelta(days=2))}")
...
def update_task_status():
    ...
    if new_status in ["Not Started", "In Progress", "Completed"]:
        ...
        print(
            f"Task {task_id} status updated from {old_status} to {new_status} in project {project_id} at {(datetime.now()+timedelta(days=2))}")
...
def display_all_projects():
    ...
    for project_id, project_info in projects.items():
        ...
        print(f"{project_id}\t\t{project_name}\t{project_status}\t\t{creation_time}\t\t{(last_update_time+timedelta(days=2))}")
def login():
    username = input("Enter username: ")
    password = input("Enter password: ")
            ...
            if password == admin_role['admin1']['password']:
                ...
                while True:
                    ...
                    if choice == 1:
                        ...
                        assign_Task(project_id, task_id, user_id)
                    elif choice == 5:
                        view_assigned_tasks()
                    elif choice == 6:
                        delete_task()
                        ...
                        Genarate_project_timeline(project_id)
                    elif choice == 8:
